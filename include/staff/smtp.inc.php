...
<form action="admin.php?t=smtp" method="post">
 ...
 <tr><td>
    <table width="100%" border="0" cellspacing=0 cellpadding=2 class="tform">
        <tr class="header"><td colspan=2><b><?= _('SMTP Server Setting (Optional)') ?></b></td></tr>
        <tr class="subheader"><td colspan=2>
             <?= _('When enabled the system will use an SMTP server rather than the internal PHP mail() function for outgoing emails.') ?><br>
             <?= _('Leave the username and password empty of the SMTP server doesn\'t require authentication') ?><br/>
             <b><?= _('Please be patient, the system will try to login to SMTP server to validate the entered login info.') ?></b></td></tr>
        <tr><th><?= _('Enable SMTP') ?></th>
            <td>
                <input type="radio" name="isenabled"  value="1"   <?=$info['isenabled']?'checked':''?> /><b><?= _('Yes') ?></b>
                <input type="radio" name="isenabled"  value="0"   <?=!$info['isenabled']?'checked':''?> /><?= _('No') ?>
        ...
        <tr><th><?= _('Encryption') ?></th>
            <td>
                 <input type="radio" name="issecure"  value="0"  
                    <?=!$info['issecure']?'checked':''?> /><?= _('None') ?>
                 <input type="radio" name="issecure"  value="1"   
                    <?=$info['issecure']?'checked':''?> /><?= _('TLS (secure)') ?>
